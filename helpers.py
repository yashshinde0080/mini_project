...
# Try to import zxing-cpp (best option - supports QR codes and barcodes, no extra DLLs needed)
try:
    import zxingcpp
...
# Try to import pyzbar (requires Visual C++ Redistributable on Windows)
try:
...
# OpenCV QR detector is always available with opencv-python
try:
...
def decode_from_camera(pil_img):
    """
    Decode QR code or barcode from camera image.
    Uses multiple methods in order of preference:
    1. zxing-cpp (best - supports QR codes and barcodes, no extra DLLs needed)
    2. pyzbar (if available - requires Visual C++ Redistributable on Windows)
    3. OpenCV QRCodeDetector (fallback - supports QR codes only)
    """
    try:
        # Method 1: Try zxing-cpp first (best option - supports both QR and barcodes)
        if ZXING_AVAILABLE:
            try:
                ...
                if results:
                    ...
                    format_str = str(result.format).replace("BarcodeFormat.", "")
                    return result.text, format_str
            except Exception:
                pass  # Fall through to next method

        # Convert PIL image to numpy array for other methods
        ...
        # Method 2: Try pyzbar (supports both QR and barcodes, but needs Visual C++ Redistributable)
        if PYZBAR_AVAILABLE:
            try:
        ...
        # Method 3: Use OpenCV QR Code Detector (built-in, no extra DLLs needed)
        if OPENCV_QR_AVAILABLE:
            try:
                # Try standard QR detector
                data, points, _ = _qr_detector.detectAndDecode(img_bgr)
                if data:
                    return data, "QRCODE"

                # Try with grayscale image for better detection
                ...
                data, points, _ = _qr_detector.detectAndDecode(enhanced)
                if data:
                    return data, "QRCODE"

            except Exception:
                pass  # QR detection failed

        # If we get here, no code was detected
        if not ZXING_AVAILABLE and not PYZBAR_AVAILABLE and not OPENCV_QR_AVAILABLE:
            st.error("No QR/Barcode decoder available. Please install zxing-cpp: pip install zxing-cpp")
...
def mark_attendance(att_col, use_mongo, student_id, status, when_dt=None, course=None, method="manual", created_by_override=None):
    ...
    # Check for duplicate within user's own attendance records only
