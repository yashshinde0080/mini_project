...
# Cookie Manager - uses COOKIE_SECRET from .env
...
def handle_url_params():
    ...
    if "reset_token" in query_params:
        ...
        reset_password.render(user_manager, token)
        return True
...
def display_session_attendance_form(session, students_col):
    ...
    # Get the session owner for student lookup
    session_owner = session.get("created_by", "admin")

    # Initialize session state for scanned code
    ...
    if camera_image is not None:
        try:
            ...
            if code_data:
                ...
                st.session_state.scanned_student_id = code_data

                # Look up student using session owner's data
                student = students_col.find_one({"student_id": code_data, "created_by": session_owner})

                if student:
                    st.info(f"👤 Student found: **{student.get('name', code_data)}**")

                    # Direct attendance marking for scanned codes
                    if st.button("✅ Mark Present Now", type="primary", key="scan_mark"):
                        ...
                        if "error" in result and result["error"] == "already":
                            ...
                            st.success(f"🎉 Attendance marked successfully for {student.get('name', code_data)}!")
                            if collections['use_mongo']:
                    ...
                    st.error("❌ Student ID not found in this session's records. Please contact your teacher.")
            else:
                st.warning("❌ No QR code or barcode detected. Please try again or use manual entry below.")
...
def handle_student_attendance_link(link_id):
    ...
    # Check link expiry
    if collections['use_mongo']:
        if link.get("expires_at") and link["expires_at"] < datetime.now():
            st.error("❌ This attendance link has expired")
            st.stop()
    else:
        if link.get("expires_at") and link["expires_at"] < datetime.now().isoformat():
            st.error("❌ This attendance link has expired")
            st.stop()
    ...
    if max_uses > 0 and current_uses >= max_uses:
        st.error("❌ This attendance link has reached its maximum usage limit")
        st.stop()

    # Get the link owner for student lookup
    link_owner = link.get("created_by", "admin")

    # Look up student using link owner's data
    student = students_col.find_one({"student_id": link["student_id"], "created_by": link_owner})
    ...
    **Date:** {date.today().strftime('%Y-%m-%d')}
    ...
    if st.button("✅ Mark Present for Today", type="primary"):
        result = mark_attendance(
            collections['attendance'],
            collections['use_mongo'],
            link["student_id"],
            ...
            created_by_override=link_owner
        )
...
if not st.session_state.auth["logged_in"]:
    if st.session_state.page == "login":
        ...
        forgot_password.render(user_manager)
    st.stop()
